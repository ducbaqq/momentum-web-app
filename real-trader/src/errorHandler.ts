import { BinanceError, BinanceErrorCode, ErrorHandler, ErrorHandlingResult } from './types.js';

export class BinanceErrorHandler implements ErrorHandler {
  ...
  private isNetworkError(code: number): boolean {
    return [
  ...
  private isRateLimitError(code: number): boolean {
    return [
  ...
  private isRequestValidationError(code: number): boolean {
    return [
  ...
  private isOrderError(code: number): boolean {
    return [
  ...
  private isAuthenticationError(code: number): boolean {
    return [
  ...
  private getLogLevel(code: number): 'info' | 'warn' | 'error' {
  ...
  private getErrorMessage(error: BinanceError): string {
...
export function parseBinanceError(error: any): BinanceError | null {
