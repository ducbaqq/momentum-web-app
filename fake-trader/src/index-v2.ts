...
class FakeTraderV2 {
  ...
  async start() {
    console.log('🚀 Starting Fake Trader V2 (Canonical Model)...');
    ...
    console.log('⏰ Fake Trader V2 scheduled to run every 1 minute');
    console.log('📊 Watching for active trading runs (version 2.0)...');
  ...
  private async handleDowntimeRecovery() {
    console.log('🔍 Checking for downtime recovery (V2)...');
  ...
  private async executeTradingCycle() {
    ...
    console.log(`\n📊 Processing ${activeRuns.length} active run(s) (V2)`);
  ...
  private async processRun(run: FakeTradeRun) {
    console.log(`\n🎯 Processing run (V2): ${run.name || run.run_id}`);
    ...
    if (currentPositions.length > run.max_concurrent_positions * 2) {
      ...
      for (const position of currentPositions) {
        ...
        if (fills.length > 0) {
          ...
          await closePositionV2(position.position_id, lastFill.price, 'safety_stop');
        }
      }
      return;
    }
...
// Start the fake trader V2
const trader = new FakeTraderV2();
trader.start().catch(error => {
  console.error('💥 Failed to start fake trader V2:', error);
  process.exit(1);
});
