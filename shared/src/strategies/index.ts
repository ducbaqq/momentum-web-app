import type { Candle, BasePosition, PositionSide, OrderType } from '../types/index.js';

export interface TradeSignal {
  symbol: string;
  side: PositionSide | 'LONG' | 'SHORT'; // Allow both for compatibility
  size: number;
  type: OrderType | 'MARKET' | 'LIMIT'; // Allow both for compatibility
  price?: number;
  stopLoss?: number;
  takeProfit?: number;
  leverage?: number;
...
export function regimeFilteredMomentumStrategy(
  ...
  if (existingPosition) {
    // Exit conditions - exit if regime breaks
    if (!inRegime) {
      signals.push({
        symbol: state.symbol,
        side: existingPosition.side === 'LONG' ? 'SHORT' : 'LONG',
